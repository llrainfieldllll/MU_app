...
# --- CONFIGURATION ---
st.set_page_config(layout="wide", page_title="Quant Scanner v18.1", page_icon="🏷️")
...
def calculate_metrics(df):
    ...
    # 2. Long Term Trend (50 & 200 Day MA)
...
# --- MAIN UI ---
def main():
    st.title("🏷️ Quant Scanner v18.1 (Labels Update)")
    ...
    if run:
        with st.spinner(f"Scanning {ticker}..."):
            ...
            # --- METRICS ROW (UPDATED WITH 20D) ---
            # Now using 6 columns to fit the extra data point
            c1, c2, c3, c4, c5, c6 = st.columns(6)
            
            c1.metric("Price", f"${cur['Close']:.2f}")
            
            # NEW: 20-Day Baseline
            c2.metric("Trend (20d)", f"${cur['Mean_20']:.2f}", delta=f"{cur['Close']-cur['Mean_20']:.2f} vs 20d")
            
            # EXISTING: 50-Day Trend
            c3.metric("Trend (50d)", f"${cur['SMA_50']:.2f}", delta=f"{cur['Close']-cur['SMA_50']:.2f} vs 50d")
            
            # UPDATED LABEL: Z-Score (20d)
            ...
            with c_chart:
                ...
                if len(valid_z) > 0:
                    ...
                    bin_count = 40
                    bin_width = (z_max - z_min) / bin_count
                    
                    fig.add_trace(go.Histogram(
                        x=valid_z, nbinsx=bin_count, 
                        histnorm=None, 
                        marker_color='#444', opacity=0.6, name='Real History'
                    ))
                    
                    # 2. Scaled Curves
                    scale_factor = len(valid_z) * bin_width
                    x_range = np.linspace(-4, 4, 100)
                    y_t = t.pdf(x_range, df=5) * scale_factor
                    ...
                    # Dynamic Text Height
                    text_y = max(y_t) * 0.8
                    fig.add_annotation(x=cur['Z_Score'], y=text_y, text="NOW", font=dict(color="#0066FF", size=14, weight="bold"))
                    
                    fig.update_layout(
                        template="plotly_white", height=350, margin=dict(t=30, b=20, l=20, r=20), 
                        xaxis_title="Z-Score Deviation (20d SMA)", # Updated Chart Axis Label too
